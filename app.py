...
class ImprovedTradingSystem:
    ...
    def _print_account_and_positions(self, prices: Dict[str, Dict[str, float]]):
        ...
        async def print_async():
            ...
            self.logger.info("\n[Account Summary]")
            self.logger.info(f"Current Balance: ${account.get('current_balance', 0):.2f} out of ${account.get('initial_balance', 0):.2f}")
            self.logger.info(f"Realized PnL: ${account.get('realized_pnl', 0):.2f} | Unrealized PnL: ${account.get('total_unrealized_pnl', 0):.2f}")
            self.logger.info(f"Total Trades: {account.get('total_trades', 0)}")
            self.logger.info(f"Profitable / Losing Trades: {account.get('profitable_trades', 0)} / {account.get('losing_trades', 0)}")
            self.logger.info(f"Win Rate: {account.get('win_rate', 0):.2f}%")
            self.logger.info(f"Daily Trades: {account.get('daily_trades_count', 0)} / {account.get('daily_trades_limit', 0)}")
            self.logger.info(f"Total Margin Used: ${account.get('total_margin_used', 0):.2f}")
            self.logger.info(f"Brokerage Charges: ${account.get('brokerage_charges', 0):.2f}")
            self.logger.info("[Open Positions]")
            for pos in positions.get('open_positions', []):
                self.logger.info(f"{pos['symbol']} | Qty: {pos['quantity']} | Entry: {pos['entry_price']} | PnL: {pos['pnl']:.2f} | PnL%: {pos['pnl_percentage']:.2f}%")
            self.logger.info("---------------------------------------------------------------")
